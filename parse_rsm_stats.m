...
fid = fopen('rsm_stats/fileList.txt', 'r');
i = 1;
while ~feof(fid)
...
load (sprintf('time_results/rsm_statistics.mat'));

for i=startFile:endFile

    [~, name, ~] = fileparts(fileNames{i});

    load (sprintf('rsm_stats/Output/HTM_SM_%s.mat', name), 'RM');
...
if endFile == 58
    save (sprintf('time_results/rsm_statistics.mat'), 'rm_access_count_trials',...
